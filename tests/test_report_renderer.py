...
def test_render_final_report_html_is_readable_and_escaped():
    decision = PortfolioDecisionOutput(
        role=Role.PORTFOLIO_MANAGER,
        ticker="SPY",
        trade_date="2026-01-15",
    ...
    report = render_final_report_html(decision, language="Korean")

    assert "<!doctype html>" in report
    assert "<title>SPY 분석 리포트</title>" in report
    assert "모의 &lt;요약&gt;입니다." in report
    assert "offline &lt;fixture&gt;" in report
    assert "https://example.test/report?a=1&amp;b=2" in report
    assert "<li>관망입니다.</li>" in report
    assert "투자 조언이 아닙니다" in report
